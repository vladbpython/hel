...
Routes by `hash(key) & mask` (the shard count is rounded up to a power of
two). The same key always reaches the same consumer.
...
let (shard_id, mut tx, mut rx) = ch.into_pairs().next().unwrap();
```

N independent SPSC channels. No shared state between shards at all. The halves
are bound `mut` on purpose: every send/recv method takes `&mut self`, so the
compiler itself enforces exactly one producer and one consumer per lane.
...
let handles: Vec<_> = inbound_rx
    ...
    .map(|(order_rx, (_, mut report_tx, mut report_rx))| {
...
```rust
// Physics has TWO producers (main loop + AI worker) - that is MPMC by
// definition, and the compiler enforces it: an SPSC sender cannot be cloned
// or shared, so a two-producer lane will not even build as SPSC.
let (physics_tx, physics_rx) = round_robin::<PhysicsCmd, 512>(1);

// AI and audio each have exactly one producer (the main loop) -> SPSC
...
let physics_rx = physics_rx.into_receivers().pop().unwrap();
let ai_physics_tx = physics_tx.clone(); // AI worker's own handle into physics
let (_, mut ai_tx, mut ai_rx) = ai_ch.into_pairs().next().unwrap();
let (_, mut audio_tx, mut audio_rx) = audio_ch.into_pairs().next().unwrap();
    ...
    loop {
        match ai_rx.recv_async().await {
            Ok(AiCmd::Think { entity }) => {
                ...
                ai_physics_tx.send(PhysicsCmd::Move(decision)).unwrap();
            }
            Err(AsyncRecvError::Disconnected) => break,
        }
    }
...
- SPSC shards have zero contention between producer and consumer (plain
  stores instead of CAS)

**Apple M2 Max (8 P-cores + 4 E-cores):**
```
8 shards -> best throughput for MPMC (8 producers, 8 consumers)
8 shards -> best for SPSC too (~250 M/s per element, ~410 M/s batched);
           16 shards is already slower: 32 threads fighting over 12 cores
```
...
**`CAP` must be a power of two, and at least 2.** Slots are indexed by
bitmask, and this is checked at compile time - an invalid capacity fails the
build instead of panicking at startup. The shard count has no such rule: MPMC
constructors round it up to a power of two themselves, and SPSC accepts any
count - `SpscShard::new(6)` on six cores gives exactly six lanes.
